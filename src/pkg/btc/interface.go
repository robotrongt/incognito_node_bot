...
type RandomClient interface {
	...
	GetNonceByTimestamp(startTime time.Time, maxTime time.Duration, timestamp int64) (int64, int64, int64, error) // return blockHeight, timestamp, nonce, int
	...
	GetTimeStampAndNonceByBlockHeight(blockHeight int64) (int64, int64, error)
}
